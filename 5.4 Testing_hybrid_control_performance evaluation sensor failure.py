# -*- coding: utf-8 -*-
"""
Created on Wed Jun 11 01:33:50 2025
...
class BasicEnv(object):
    ...
    def step(self, action):
        ...
        return self.nodedepth, self.rainfalldata, self.state, done, info, cso_volume, current_time 
    ...
    def reset(self, testing_year):
        ...
        return self.nodedepth, self.rainfalldata, self.state, self.T
...
def get_reward_per_year(network_params, testing_year):
    ...
    Nodedepth, Rainfalldata, Storm_state, EP_LEN = storm_env.reset(testing_year)
    ...
    failure_counters = [0, 0, 0, 0]
    failure_totals = [0, 0, 0, 0]
    ...
    for t in range(EP_LEN):
        if t == 0:
            ...
            input_state = []
            for i in range(4):
                if failure_counters[i] > 0:
                    failure_counters[i] -= 1
                    failure_totals[i] += 1
                    input_state.append(last_step_state[i])
                else:
                    a = np.random.uniform(0, 1)
                    if a <= failure_probability:
                        failure_counters[i] = failure_steps
                    input_state.append(Storm_state[i])
        ...
        # control logic
        if t <= 11 or np.max(failure_totals) <= 132:
            Action = get_action(network_params, input_state)
        else:
            
            policy_input = np.array([[ 
                Historical_rainfalldata[t-11][0], Historical_rainfalldata[t-11][1], Historical_rainfalldata[t-11][2], Historical_rainfalldata[t-11][3],
                Historical_rainfalldata[t-10][0], Historical_rainfalldata[t-10][1], Historical_rainfalldata[t-10][2], Historical_rainfalldata[t-10][3],
            ...
            Action = forward_looking_control_model.predict(policy_input)[0]

        Nodedepth, Rainfalldata, New_Storm_state, done, info, CSO_volume, current_time  = storm_env.step(Action)
...
if __name__ == "__main__":
    ...
    workbook_curve = xlsxwriter.Workbook('Performance comparison/Hybrid control_sensor failure_Overall' +'.xlsx')
    ...
    for mmm in range (len(TESTING_YEARS)):
        ...
        workbook_year = xlsxwriter.Workbook('Performance comparison/Hybrid control_sensor failure_Overall_year ' + str(testing_year) +'.xlsx')
